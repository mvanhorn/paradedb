...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.MatchDisjunction(item.Description, "shoes", Pdb.Boost(2))
        || EF.Functions.MatchDisjunction(item.Category, "footwear")
    )
    .Select(item => new
    {
        item.Id,
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.MatchDisjunction(item.Description, "shose", Pdb.Fuzzy(2), Pdb.Boost(2))
    )
    .Select(item => new
    {
        item.Id,
...
```cs EF Core
var results = await dbContext
    .MockItems.Where(item => EF.Functions.MatchDisjunction(item.Description, "shoes", Pdb.Const(1)))
    .Select(item => new
    {
        item.Id,
        Score = EF.Functions.Score(item.Id),
