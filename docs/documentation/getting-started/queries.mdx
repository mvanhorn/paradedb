...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.MatchDisjunction(item.Description, "running shoes") && item.Rating > 2
    )
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.MatchConjunction(item.Description, "running shoes") && item.Rating > 2
    )
    .OrderBy(item => item.Rating)
    .Select(item => new { item.Description, item.Rating, item.Category })
    .Take(5)
...
```cs EF Core
var results = await dbContext
    .MockItems.Where(item => EF.Functions.MatchDisjunction(item.Description, "running shoes"))
    .OrderBy(item => item.Rating)
    .Select(item => new { item.Description, item.Rating, item.Category })
    .Take(5)
    .ToListAsync();
