...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Match(item.Description, Pdb.Proximity("sleek").Within(1, "shoes"))
    )
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Match(item.Description, Pdb.Proximity("sleek").WithinOrdered(1, "shoes"))
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
...
await db
  ...
  .where(
    search.proximity(
      mockItems.description,
      search.proxRegex("sl.*").within(1, "shoes"),
    ),
  );
```
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Match(item.Description, Pdb.ProximityRegex("sl.*").Within(1, "shoes"))
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Match(item.Description, Pdb.ProximityRegex("sl.*", 100).Within(1, "shoes"))
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
...
await db
  ...
  .where(
    search.proximity(
      mockItems.description,
      search.proxArray("sleek", "white").within(1, "shoes"),
    ),
  );
```
...
await db
  ...
  .where(
    search.proximity(
      mockItems.description,
      search.proxArray(search.proxRegex("sl.*"), "white").within(1, "shoes"),
    ),
  );
```
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Match(
            item.Description,
            Pdb.ProximityArray(Pdb.ProximityRegex("sl.*"), Pdb.Proximity("white")).Within(1, "shoes")
        )
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Match(
            ...
            Pdb.Proximity("sleek")
                .Within(1, "running")
                .Within(2, Pdb.ProximityArray(Pdb.Proximity("sneakers"), Pdb.ProximityRegex("sho.*")))
        )
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
