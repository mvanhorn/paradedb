...
```cs EF Core
var fuzzyOrRows = await dbContext
    .MockItems.Where(item =>
        EF.Functions.MatchDisjunction(item.Description, "runing shose", Pdb.Fuzzy(2))
    )
    .Select(item => new { item.Id, item.Description })
    .Take(5)
    .ToListAsync();

var fuzzyAndRows = await dbContext
    .MockItems.Where(item =>
        EF.Functions.MatchConjunction(item.Description, "runing shose", Pdb.Fuzzy(2))
    )
    .Select(item => new { item.Id, item.Description })
    .Take(5)
    .ToListAsync();

var fuzzyTermRows = await dbContext
    .MockItems.Where(item => EF.Functions.Term(item.Description, "shose", Pdb.Fuzzy(2)))
    .Select(item => new { item.Id, item.Description })
    .Take(5)
    .ToListAsync();
```
...
var results = await dbContext
    .MockItems.Where(item => EF.Functions.Term(item.Description, "rann", Pdb.Fuzzy(1, true)))
    .Select(item => new { item.Id, item.Description })
    .Take(5)
    .ToListAsync();
```
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.MatchConjunction(item.Description, "slee rann", Pdb.Fuzzy(1, true))
    )
    .Select(item => new { item.Id, item.Description })
    .Take(5)
    .ToListAsync();
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Term(item.Description, "shose", Pdb.Fuzzy(1, false, true))
    )
    .Select(item => new { item.Id, item.Description })
    .Take(5)
    .ToListAsync();
