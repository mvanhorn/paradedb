...
var results = await dbContext
    .MockItems.Where(item => EF.Functions.Phrase(item.Description, "running shoes"))
...
await db
  ...
  .from(mockItems)
  .where(search.phrase(mockItems.description, search.slop("shoes running", 2)));
```

```python Django
from paradedb import ParadeDB, Phrase, Slop

MockItem.objects.filter(
    description=ParadeDB(Phrase(Slop('shoes running', 2)))
).values('description', 'rating', 'category')
```
...
```cs EF Core
var results = await dbContext
    .MockItems.Where(item => EF.Functions.Phrase(item.Description, "shoes running", Pdb.Slop(2)))
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
...
```python Django
from paradedb import ParadeDB, Phrase, Tokenized, Tokenizer

MockItem.objects.filter(
    description=ParadeDB(Phrase(Tokenized('running shoes', Tokenizer.whitespace())))
).values('description', 'rating', 'category')
```
...
stmt = (
    select(MockItem.description, MockItem.rating, MockItem.category)
    .where(search.phrase(MockItem.description, "running shoes", tokenizer=tokenizer.whitespace()))
)
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Phrase(item.Description, new[] { "running", "shoes" })
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
...
await db
  ...
  .from(mockItems)
  .where(
    search.phrase(mockItems.description, search.slop(["shoes", "running"], 2)),
  );
```
...
var results = await dbContext
    .MockItems.Where(item =>
        EF.Functions.Phrase(item.Description, new[] { "shoes", "running" }, Pdb.Slop(2))
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
